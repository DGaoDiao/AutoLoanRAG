"""
OCR介绍:
    简单说就是'光学字符识别'技术, Optical Character Recognition, 能让电脑'看懂'图片里的文字,
    把图片上的手写体, 印刷体文字转换成可编辑的文本 -> 例如: 照片里的笔记,扫描版pdf的文字等.
"""
...
from rag_qa.document_loaders.finance_ocr import get_ocr, normalize_finance_text, recognize_finance_image
        ...
        for i, page in enumerate(doc):
            ...
            for img in img_list:
                ...
                if xref := img.get("xref"):
                    ...
                    if len(re.sub(r"\s+", "", text)) >= 40:
                        image_text = recognize_finance_image(ocr, img_array)
